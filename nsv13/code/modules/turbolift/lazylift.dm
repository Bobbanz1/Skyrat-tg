...
/obj/machinery/lazylift
	...
	var/static/list/moving_blacklist = typecacheof(list(
		/obj/machinery/lazylift,
...
/obj/machinery/lazylift/master/proc/move_platform(targetDeck)
	...
	for(var/turf/T as() in platform_location.platform)
		...
		for(var/atom/movable/AM as() in T.contents)
			if(AM.anchored && moving_blacklist[AM.type]) //To stop the lift moving itself and its components
				continue
			AM.forceMove(newT)
