...
/datum/ship_weapon/proc/special_fire(atom/target, ai_aim = FALSE)
	...
	if(!requires_physical_guns)
		if(special_fire_proc)
			call_async(source=holder, proc_type=special_fire_proc, arguments=list(target=target, ai_aim=ai_aim, burst=burst_size)) //WARNING: The default behaviour of this proc will ALWAYS supply the target method with the parameter "target". Override this proc if your thing doesnt have a target parameter!
		else
			weapon_sound()
